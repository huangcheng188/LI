...
void DoUartCommand(AvPort *port)
{
    ...
    if CommandNameIs("A") /* ARC */
    {
        ...
        if(AudioStatus == 0)
        {
            AudioStatus = 1;
#if (AvCecPortIndex == 5)
            if((&port[1])->content.cec->EnableAudioAmplifier != AV_CEC_AMP_ENABLED)
                (&port[1])->content.cec->EnableAudioAmplifier = AV_CEC_AMP_TO_ENABLE;
            if((&port[1])->content.cec->EnableARC != AV_CEC_ARC_INITIATED)
                (&port[1])->content.cec->EnableARC = AV_CEC_ARC_TO_INITIATE;
#elif (AvCecPortIndex == 0)
            if((&port[0])->content.cec->EnableAudioAmplifier != AV_CEC_AMP_ENABLED)
                (&port[0])->content.cec->EnableAudioAmplifier = AV_CEC_AMP_TO_ENABLE;
            if((&port[0])->content.cec->EnableARC != AV_CEC_ARC_INITIATED)
                (&port[0])->content.cec->EnableARC = AV_CEC_ARC_TO_INITIATE;
#endif
        ...
        else
        {
            AudioStatus = 0;
#if (AvCecPortIndex == 5)
            if((&port[1])->content.cec->EnableAudioAmplifier != AV_CEC_AMP_DISABLED)
                (&port[1])->content.cec->EnableAudioAmplifier = AV_CEC_AMP_TO_DISABLE;
#elif (AvCecPortIndex == 0)
            if((&port[0])->content.cec->EnableAudioAmplifier != AV_CEC_AMP_DISABLED)
                (&port[0])->content.cec->EnableAudioAmplifier = AV_CEC_AMP_TO_DISABLE;
#endif
